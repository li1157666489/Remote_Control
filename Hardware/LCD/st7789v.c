...
void ST7789V_DrawArea(uint16_t x_start, uint16_t y_start, uint16_t width, uint16_t height, uint8_t *px_map)
{
    /* 参数边界检查 */
    if (x_start >= ST7789V_WIDTH || y_start >= ST7789V_HEIGHT || width == 0 || height == 0 || px_map == NULL) {
        return;
    }
    ...
    if (x_end >= ST7789V_WIDTH) {
        x_end = ST7789V_WIDTH - 1;
        width = x_end - x_start + 1;
    }
    if (y_end >= ST7789V_HEIGHT) {
        y_end = ST7789V_HEIGHT - 1;
        height = y_end - y_start + 1;
    }
    
    /* 设置显示窗口（仅设置一次，而不是每个像素都设置） */
    ST7789V_SetAddressWindow(x_start, y_start, x_end, y_end);
    
    /* 计算需要传输的总字节数 */
    uint32_t total_bytes = (uint32_t)width * (uint32_t)height * 2; /* RGB565: 每个像素2字节 */
    
    /* 批量发送数据到屏幕（高效方式） */
    ST7789V_WriteData(px_map, total_bytes);
}
