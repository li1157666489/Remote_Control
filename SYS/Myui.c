...
void myui_update_control(int16_t throttle, int16_t yaw, int16_t pitch, int16_t roll)
{
    ...
    /* 摇杆最大偏移像素 */
    #define JOYSTICK_MAX_OFFSET 35

    /* 更新左摇杆位置（油门垂直，偏航水平） */
    /* ADC范围0-4095，中心点2048 */
    /* 映射到 ±35像素 */
    int32_t throttle_pos = -((int32_t)(throttle - 2048) * JOYSTICK_MAX_OFFSET) / 2048;
    int32_t yaw_pos = ((int32_t)(yaw - 2048) * JOYSTICK_MAX_OFFSET) / 2048;

    /* 限制范围，防止越界 */
    if (throttle_pos < -JOYSTICK_MAX_OFFSET) throttle_pos = -JOYSTICK_MAX_OFFSET;
    if (throttle_pos > JOYSTICK_MAX_OFFSET) throttle_pos = JOYSTICK_MAX_OFFSET;
    if (yaw_pos < -JOYSTICK_MAX_OFFSET) yaw_pos = -JOYSTICK_MAX_OFFSET;
    if (yaw_pos > JOYSTICK_MAX_OFFSET) yaw_pos = JOYSTICK_MAX_OFFSET;

    /* 只在值变化时更新摇杆 */
    if (last_throttle != throttle || last_yaw != yaw) {
        if (joystick_knob_left) {
            lv_obj_align(joystick_knob_left, LV_ALIGN_CENTER, (lv_coord_t)yaw_pos, (lv_coord_t)throttle_pos);
        }
    }

    /* 更新右摇杆位置（俯仰垂直，横滚水平） */
    int32_t pitch_pos = -((int32_t)(pitch - 2048) * JOYSTICK_MAX_OFFSET) / 2048;
    int32_t roll_pos = ((int32_t)(roll - 2048) * JOYSTICK_MAX_OFFSET) / 2048;

    /* 限制范围，防止越界 */
    if (pitch_pos < -JOYSTICK_MAX_OFFSET) pitch_pos = -JOYSTICK_MAX_OFFSET;
    if (pitch_pos > JOYSTICK_MAX_OFFSET) pitch_pos = JOYSTICK_MAX_OFFSET;
    if (roll_pos < -JOYSTICK_MAX_OFFSET) roll_pos = -JOYSTICK_MAX_OFFSET;
    if (roll_pos > JOYSTICK_MAX_OFFSET) roll_pos = JOYSTICK_MAX_OFFSET;

    /* 只在值变化时更新摇杆 */
    if (last_pitch != pitch || last_roll != roll) {
        if (joystick_knob_right) {
            lv_obj_align(joystick_knob_right, LV_ALIGN_CENTER, (lv_coord_t)roll_pos, (lv_coord_t)pitch_pos);
        }
    }
 ...
 /*   if (throttle != last_throttle) {
        ...
        int16_t rotor_speed = throttle / 20;
        for (int i = 0; i < 4; i++) {
...
void myui_update_status(uint8_t battery, int8_t signal, uint8_t speed)
{
    ...
    if (battery != last_battery) {
        ...
        if (battery_bar) {
            lv_bar_set_value(battery_bar, battery, LV_ANIM_OFF);
        }
        if (battery_label) {
    ...
    if (speed != last_speed) {
        last_speed = speed;
        if (speed_slider) {
            lv_slider_set_value(speed_slider, speed + 1, LV_ANIM_OFF);
        }
    }
}
